...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.filter {
      article => article.text.contains(lang)
    }.count().toInt
  ...
  private def findLangs(langs: List[String], article: WikipediaArticle) =
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(article => {
      findLangs(langs, article).map(lang => (lang, article))
    }).groupByKey
